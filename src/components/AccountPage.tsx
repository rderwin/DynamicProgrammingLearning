...
export default function AccountPage({ userData, onReset, moduleConfigs, onHome }: Props) {
  ...
  return (
    <div className="max-w-lg mx-auto animate-fade-in">
      {/* Back */}
      <button onClick={onHome} className="flex items-center gap-1.5 text-sm text-slate-400 hover:text-slate-600 transition-colors mb-6">
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm mb-6 text-center">
        {user.photoURL && (
          <img src={user.photoURL} alt="" className="w-16 h-16 rounded-full mx-auto mb-3 border-2 border-slate-200" referrerPolicy="no-referrer" />
        )}
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm mb-6">
        <h3 className="text-sm font-semibold text-slate-700 mb-4">Progress by module</h3>
      ...
      <div className="space-y-3">
        {!showResetGate ? (
          <button onClick={() => setShowResetGate(true)} className="w-full text-left bg-white border border-slate-200 rounded-xl px-5 py-4 hover:border-red-300 transition-colors group">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400">Clear all modules, practice, and saved code</p>
              </div>
            </div>
          </button>
        ) : (
          <div className="bg-red-50 border border-red-200 rounded-xl p-5 animate-fade-in">
            ...
            <div className="flex gap-2">
              ...
              <button onClick={() => setShowResetGate(false)} className="px-4 py-2 bg-white text-slate-600 border border-slate-200 rounded-lg text-xs font-semibold hover:bg-slate-50 transition-colors">Cancel</button>
            </div>
          </div>
        )}

        <button onClick={signOut} className="w-full text-left bg-white border border-slate-200 rounded-xl px-5 py-4 hover:border-slate-300 transition-colors">
          <div className="flex items-center gap-3">
