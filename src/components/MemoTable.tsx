...
export default function MemoTable({ n, memo, lastWritten, lastRead }: Props) {
  ...
  return (
    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl p-5 shadow-sm dark:shadow-slate-900/50">
          ...
          return (
            <div key={i} className="flex flex-col items-center gap-1.5">
              <div className={`text-[10px] font-mono font-medium transition-colors duration-200 ${
                isWritten ? "text-green-600" : isRead ? "text-amber-600" : "text-slate-400 dark:text-slate-500"
              }`}>{i}</div>
              <div
                className={`
