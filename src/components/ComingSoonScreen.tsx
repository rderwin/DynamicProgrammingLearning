...
export default function ComingSoonScreen({ module: mod, onBack }: Props) {
  return (
    <div className="max-w-lg mx-auto text-center animate-fade-in">
      <button
        onClick={onBack}
        className="flex items-center gap-1.5 text-sm text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300 transition-colors mb-8 mx-auto"
      >
      ...
      <h2 className="text-2xl font-bold text-slate-900 dark:text-slate-100 mb-2">{mod.title}</h2>
      <p className="text-slate-500 dark:text-slate-400 mb-6">{mod.description}</p>
